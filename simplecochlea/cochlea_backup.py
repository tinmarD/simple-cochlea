...
class LIFBank:
    ...
    def __init__(self, n_channels, fs, tau, v_thresh=0.020, v_spike=0.035, t_refract=[], v_reset=[], inhib_type=[],
        ...
        self.inhib_vect = np.float64(inhib_vect) if inhib_type else []
    ...
    def filter_signal(self, input_sig, v_init=[], t_start=0, t_last_spike=[]):
        ...
        output_sig, t_spikes, chan_spikes = lif_filter_cy(self.fs, input_sig, self.refract_period, self.t_refract,
                                                          self.tau, self.v_thresh, self.v_spike, self.v_reset,
                                                          v_init, self.adaptive_threshold, self.tau_j, self.alpha_j,
                                                          self.omega, t_start, t_last_spike)
        spikes = np.vstack([t_spikes, chan_spikes]).T
        return output_sig, spikes, []

    def filter_signal_old(self, input_sig, do_plot=0, v_init=[], t_start=0, t_last_spike=[], n_processes=[]):
        input_sig = np.array(input_sig).squeeze()
        if len(input_sig.shape) == 1:
            input_sig = np.tile(input_sig, (self.n_channels, 1))
        if not input_sig.shape[0] == self.n_channels:
